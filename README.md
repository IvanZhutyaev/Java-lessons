...
### 4) YAGNI - You are not gonna need it - "Вам это не понадобится"

- <b>Реализуйте только то, что нужно здесь и сейчас, а не в теории, что оно пригодится в будущем
- Подчищайте ненужный код (найдите через Git историю по надобности)
- Программист не должен добавлять новый функционал, о котором его не просят (благими намерениями без должной проверки вы только добавите багов)</b>

### 5) SOLID
![img.png](images/5413821494137130129.jpg)
 - <b>S - Каждый объект должен иметь одну обязанность, эта обязанность должна быть полностью реализована классом
 - O - Классы, функции, модули и т.д. должны быть открыты для расширения, но закрыты для изменения
 - L - Функции, которые используют базовый тип должны иметь возможность использовать подтипы базового
 - I - Каждый класс должен содержать только необходимые ему методы. Вместо одного большого интефейса создаём несколько маленьких-специализированных
...
```
<b>Создаёт копию, не изменяет оригинал!!!</b><br>
<b>При работе со Stream API важно понимать, что все операции с потоками бывают либо терминальными, либо промежуточными.<br>Промежуточные возвращают трансформированный поток.<br>Конечные или терминальные всегда возвращают конечный результат.<br>ВСЕ ПОТОКИ ПРОИЗВОДЯТ ВЫЧИСЛЕНИЯ, В ТОМ ЧИСЛЕ, В ПРОМЕЖУТОЧНЫХ ОПЕРАЦИЯХ, ТОЛЬКО ТОГДА, КОГДА К НИМ ПРИМЕНЯЕТСЯ ТЕРМИНАЛЬНАЯ ОПЕРАЦИЯ</b>

### Отличие коллеций от потоков
...
```
<b>Важно, что после использования терминальных операций другие терминальные или промежуточные операции к этому же потоку не могут быть применены</b><br>

<b>Фактически жизненный цикл потока проходит следующие три стадии:</b>
- <b>Создание потока
- Применение к потоку ряда промежуточных операций
- Применение к потоку терминальной операции и получение результата</b>

<b>Для создания потоков IntStream, DoubleStream, LongStream можно использовать соответсвующие перегруженные версии этого метода.<br>Или же просто исопльзовать .stream как в примере</b>
...
## Тип Optional
<b>ПОЗВОЛЯЕТ ИЗБЕГАТЬ NULL-ЗНАЧЕНИЕ. После выполнения операции с помощью метода get() объекта Optional мы можем получить его значение</b>
...
<b>orElse - позволяет определить альтернативное значение, которое будет возвращаться , если Optional не получит из потока какого-нибудь значения</b>
![img.png](images/irejgifrng.png)
...
<b>ifPresent() - определяет действия со значением в Optional, если значение имеется</b>
![img.png](images/аоьтвыжпткурпьл.png)


<br>

<b>ifPresentOrElse() - позволяет определить альтернативную логику на случай, если значение в Optional отсутствует</b>
![img.png](images/щкапщзкеуьпрщзекщз.png)
...
![img.png](images/хдхдеклрдэжьуеклзрь.png)
...
### Метод interrupt
<b>Метод interrupt - ещё один способ вызова завершения или прерывания потока. Вызов этого метода устанавливает потоку статус, что он прерван. Сам метод возвращает true, если поток может быть прерван, в ином случае возвращается false. При этом сам вызов этого метода НЕ завершает поток, он только устанавливает статус: в частности, метод isInterrupted() класса Thread будет возвращать значение true. Мы можем проверить значение возвращаемое данными методом и произвести некоторые действия</b>
![img.png](images/еркьщзекущлзренрлщ.png)
